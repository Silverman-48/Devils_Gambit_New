...
function rpgResolveEnemyState() {
  ...
  if (isAttack) {
    finalState = 'attack';
    rpgCurrentAttackChance = Math.max(0.0, rpgCurrentAttackChance - (RPG_PRESET.enemyAttackChanceDecrement ?? 0.05));
  } else {
...
function rpgResolveGambit(gs, dg) {
  ...
  if (gs.enemyState === 'neutral') {
    ...
    if (won) {
      ...
      else if (diff < 0) {                                combatResult = 'miss-guard';   }
      else               {                                combatResult = 'draw';        }
    } else {
      combatResult = 'miss-guard';
    }
  } else {
...
function rpgResolveBlank(gs) {
  ...
  if (gs.enemyState === 'neutral') {
    ...
    const enemyDamage = Math.floor(gs.tableCard.numValue);
    const newEnemyHP  = Math.max(0, gs.enemyHP - enemyDamage);
    return {
      type: 'strike',
      enemyDamage, newEnemyHP, attackValue: null,
