      ...
      // ── Combat result display ─────────────────────────────────────────────
        ...
        (() => {
          ...
          const cases = {
            ...
            'miss-guard':       ['🛡️', 'Repelled!',        `Guard deflects`],
          };
...
function RpgSettingsPanel({
  ...
  const [activePresetId,    setActivePresetId]    = React.useState(RPG_PRESETS[0]?.id ?? null);
  ...
  const combatContent = () => {
    ...
    const atkDecrPct = Math.round((get('enemyAttackChanceDecrement', 0.05)) * 100);
    ...
    return e('div', null,
      ...
      sliderRow('Attack Chance Decay', 0,  25, 1, atkDecrPct, '−' + atkDecrPct + '%',
        ev => set('enemyAttackChanceDecrement', Number(ev.target.value) / 100)),
      ...
      sliderRow('Guard Chance Decay',   0,  25, 1, defDecrPct, '−' + defDecrPct + '%',
