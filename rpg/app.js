...
function RpgApp({ onReturnToMenu }) {
  ...
  // ── Animation helpers ───────────────────────────────────────────────────────
  // Timing constants — match the CSS keyframe durations with a safety buffer
  // so the .deal class stays applied until the animation has fully finished.
  ...
  const continueGame = useCallback(() => {
    ...
    // Disappear: play sound and start the dealOut animation simultaneously.
    // Both cards fade-and-drop together (the hand card fades face-up — we
    // don't flip it back, since "leaving" semantically replaces flip-back).
    if (window.SOUND) window.SOUND.playCardDisappear();
    setLeaving(true);
    setTimeout(() => {
      const ng = drawNext(currentGs);
      // Atomic state swap: new cards mount fresh (key changes via card.id),
      // so the .revealed → not-revealed change can't fire a stray flip-back
      // on the new card — it's a brand-new component instance.
      setGs(ng);
